...
def assemble_sdf_local_contributions(
    ...
    *,
    ...
    phi_target_values = np.asarray(phi_target, dtype=float)
    ...
    for cell in range(num_cells):
        ...
        phi_target_local: object = phi_target
        if phi_target_values.ndim == 1 and phi_target_values.shape == phi_values.shape:
            phi_target_local = phi_target_values[phi_dofs]
        ...
        A = np.eye(tdim, dtype=float) + np.tensordot(u_local, dA_du, axes=(0, 0))
